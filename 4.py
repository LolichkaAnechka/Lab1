...
def knapsack(capacity, weight):
    """Function to count the maximum weight you can fit in knapsac"""
    numb_of_bars = len(weight)
    subp_list = [[0 for x in range(capacity + 1)] for x in range(numb_of_bars + 1)]
    #list that contains all subproblems exist
    for i in range(numb_of_bars + 1):
        for value in range(capacity + 1):
            if i == 0 or value == 0:
                subp_list[i][value] = 0
            #filling the first row with zeros to fill next rows
            #if the value we consider is lower then the weight of any bar we consider
    ...
    return subp_list[numb_of_bars][capacity]
